...
可以直接在桌面聊天框中说「每个工作日 09:00 提醒我要开会」。Agent 会先把自然语言拆成任务内容、执行时间和重复频率，再持久化到本机调度器。任务到点后走专用的 `scheduled` 执行模式，模型只生成这一次应该交付的内容，不会重新把任务文本当成创建提醒的普通请求。执行结果会回到创建任务的桌面会话；应用窗口关闭时会先保存在待投递队列，重新打开后写入会话。
